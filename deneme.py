...
def MainFonksiyon(user_message,attribute,dict):
    def func(urun):
        ...
        if dict.get(urun) == None:
            ...
            print(text) #dispatcher.utter_message(text =text)
        else:
            print(f"{(attribute).title()}")    #dispatcher.utter_message(text ="**Poliçe Süresi;**")
            print(dict.get(urun)) #dispatcher.utter_message(text =dict_police_suresi.get(urun))
    ...
    if matched_key is not None:
        ...
        for i in dict.values():
            print(i) #dispatcher.utter_message(text =i)  
    return []       
